...
gradient_values = sess.run(grads, {x: np.ones(shape=(1, 10,)), y: np.ones(shape=(1, 1))}).flatten()
print('grads = ', gradient_values)

weight_values = sess.run(tf.trainable_variables(), {x: np.ones(shape=(1, 10,)), y: np.ones(shape=(1, 1))})[0].flatten()
